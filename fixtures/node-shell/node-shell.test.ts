...
test('GITHUB_PATH prepends a path for subsequent steps (ternary branch)', async () => {
  const result = await actharness('./action.yml').run({
    inputs: { 'should-fail': 'false' },
  });
...
test('GITHUB_PATH: explicit if/else — true branch covered, else branch uncovered', async () => {
  const result = await actharness('./action.yml').run({
    inputs: { 'should-fail': 'false' },
  });

  expect(result).toHaveOutput('has-custom-bin-explicit', 'true');
});
...
test('mockNetwork intercepts fetch with a string URL', async () => {
  actharness.mockNetwork('https://api.example.com/data', 200, { value: 'from-mock' });

  const result = await actharness('./action-fetch.yml').run();

  expect(result).toHaveSucceeded();
  expect(result).toHaveOutput('value', 'from-mock');
});

test('mockNetwork returns non-200 status and step sets failure', async () => {
