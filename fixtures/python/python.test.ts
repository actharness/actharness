...
const pythonAvailable = binaryExists('python');
...
test('shell: python works — basic output via the python binary alias', async () => {
  if (!pythonAvailable) return;

  const result = await actharness('./action-python.yml').run({
    inputs: { name: 'Alice', 'should-fail': 'false' },
  });
