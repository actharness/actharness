...
test('Write-Warning appears in stderr, Write-Host appears in stdout', async () => {
  const result = await actharness('./action.yml').run({
    inputs: { 'should-fail': 'false' },
  });

  expect(result.step('write-warning')).toHaveStderrContaining('something looks off');
  expect(result.step('write-warning')).toHaveStdoutContaining('host output');
  expect(result.step('write-warning')).not.toHaveStdoutContaining('something looks off');
});
