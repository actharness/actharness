import type { ExprValue, ExpressionContexts, Token, Ast, TokenKind } from './types.js';
import { tokenize as _tokenize } from './tokenizer.js';
import { parse as _parse } from './parser.js';
import { evaluate as _evaluate, coerceToString } from './evaluator.js';

export { ExpressionError } from './tokenizer.js';
export type { ExprValue, ExpressionContexts, Token, TokenKind, Ast };
...
export function evaluateTemplate(input: string, contexts: ExpressionContexts = {}): ExprValue {
  ...
  const nonEmpty = segments.filter(
    (s) => s.kind === 'expr' || (s.kind === 'text' && s.text.length > 0),
  );
