// ActionMock + MockRegistry — per-Action instance, never global.
...
class ActionMockImpl_ implements ActionMock {
  ...
  mockOutputs(outputs: Record<string, string>): this {
    this._def = { ...this._def, outputs };
    return this;
  }

  mockConclusion(c: 'success' | 'failure'): this {
    this._def = { ...this._def, conclusion: c };
    return this;
  }
  ...
  mockImplementationOnce(impl: ActionMockImpl): this {
    this._onceQueue.push(impl);
    return this;
  }

  clear(): void {
    this.calls.length = 0;
    this._onceQueue.length = 0;
  }
...
export class MockRegistry {
  ...
  mock(ref: string, def?: ActionMockDef | ActionMockImpl): ActionMock {
    ...
    if (typeof def === 'function') {
      m.mockImplementation(def);
    } else if (def) {
      if (def.outputs) m.mockOutputs(def.outputs);
      if (def.conclusion) m.mockConclusion(def.conclusion);
    }
  ...
  mockGitHubApi(routes: GitHubApiRoutes): void {
    this._githubApiRoutes = { ...this._githubApiRoutes, ...routes };
  }

  get githubApiRoutes(): GitHubApiRoutes {
    return this._githubApiRoutes;
  }

  hasMock(ref: string): boolean {
    return this._mocks.has(normalizeRef(ref));
  }

  async invoke(
    ref: string,
    callEnv: { with: Record<string, string>; env: Record<string, string> },
  ): Promise<ActionMockDef> {
  ...
  reset(): void {
    this._mocks.clear();
    this._githubApiRoutes = {};
  }

  clearCalls(): void {
    for (const m of this._mocks.values()) m.clear();
  }
}
