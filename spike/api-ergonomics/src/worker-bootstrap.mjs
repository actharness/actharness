/**
 * Worker bootstrap — runs inside the worker_thread before the action entrypoint.
 * Adapted from spike/sandbox/src/worker-bootstrap.mjs with findings applied:
 *   §1 (process.exit trap), §2 (same-instance undici patching).
 */
...
// 1. Apply env from workerData (INPUT_*, GITHUB_OUTPUT, etc.)
for (const [k, v] of Object.entries(workerData.env)) {
  process.env[k] = String(v);
}

// 2. Trap process.exit so it doesn't kill the test runner process (finding §1).
...
// 3. Mount undici MockAgent from the action's own undici instance (finding §2).
